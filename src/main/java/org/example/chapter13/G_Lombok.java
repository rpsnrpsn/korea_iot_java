...
// == 롬복(Lombok) 라이브러리 == //
// : 자바에서 반복적으로 작성해야하는 보일러플레인트(boilerplate) 코드를 줄여주는 라이브러리
// - 이노테이션(코드 주석)을 사용하여 코드를 간결하게 만들어주는 역할
...
// - 주로 Maven 빌드도구와 함께 사용되지만, Gradle 에서도 사용
...
// 4. @Data 어노테이션
// @Getter, @Setter, @ToString, @RequiredConstructor, @EqualsAndHashCode 어노테이션
//      한 번에 적용하는 어노테이션
...
// - 필드값을 모두 메서드 체이닝으로 가져옴 (필수값 포함 메서드 체이닝으로 구성)
// - 메서드 체이닝 이름은 필드명
// >> 필수값을 체이닝에서 누락 시 오류
