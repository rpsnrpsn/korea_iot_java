package org.example.z_project.phr_soultion;

/*
* == PHR (개인 건강 기록, Personal Health Record) 솔루션 ==
*
* 1. 프로젝트 개요
*   - 개인 건강 기록(PHR) 솔루션을 위한 간단한 콘솔 기반 공용 프로그램
*   - 환자의 기본 정보를 관리, 환자의 건강 기록 등록, 수정, 조회 기능
*   - 특정 조건에 맞는 건강 기록을 필터링하고 집계 가능
*
* 2. 주요 기능 & 비지니스 로직
...
*           >> App.java가 사용자 입력 처리, 메뉴 출력, 흐름 제어 모두 담당 -> SRP 위반 (단일 책입 원칙)
*           >> handler + App.java로 구분
* */

import org.example.z_project.phr_soultion.controller.HealthRecordController;
import org.example.z_project.phr_soultion.controller.PatientController;
...
import org.example.z_project.phr_soultion.handler.InputHandler;
import org.example.z_project.phr_soultion.handler.MenuPrinter;
import org.example.z_project.phr_soultion.util.DateValidator;
...
public class App {
    ...
    private static boolean processChoice(int choice) {
        switch (choice) {
            ...
            case 4: {
                long id = InputHandler.getIdInput();
                 PatientUpdateRequestDto requestDto = InputHandler.updateRequestDto();
    ...
    public static void main(String[] args) {
        try {
            while (true) {
                ...
                if (!processChoice(choice)) break; // processChoice 반환값이 false면 로직 종료
            }
