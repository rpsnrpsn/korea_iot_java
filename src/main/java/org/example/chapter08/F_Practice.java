...
abstract class MobileApp {
    // 추상 메서드: 실행에 대한 동작을 정의
    // - 각 앱 마다 '고유한 실행'을 정의 (상속받은 클래스에서 구현)
...
class SocialApp extends MobileApp {
   String name; // 인스턴스 필드 (각 개체가 가질 이름 ex. 인스타그램, 카카오톡 등)
        ...
        if (socialApp instanceof GameApp) {
            ...
            for (MobileApp app: apps) {
                ...
                if (app instanceof SocialApp) {
                    // 다운 캐스팅
                    SocialApp specificSoocial = (SocialApp) app;
                    specificSoocial.shareContent("Files");
                }
            }
        }
