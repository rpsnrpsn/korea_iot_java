...
// - 추상 메서드: 만드시 자식 클래스에서 오버라이딩 (구현부를 명시)
...
class Dog extends Animal {
    ...
    @Override
    void eat() {
        System.out.println("강아지는 개껌을 먹었습니다.");
    }
...
//        Human human = new Human(); // 'Human'은(는) abstract이며, 인스턴스화할 수 없습니다
        ...
        //      >> 단, 자신만이 가진 멤버(필드 & 매서드)에 접근 불가
        Animal animalDog = new Dog();
        animalDog.makeSound(); // 멍멍
        animalDog.eat();; // 강아지는 개껌을 먹었습니다. - 오버라이딩된 결과가 출력
//        animalDog.bark();
    }
}
